        ...
        <article>
          <div className="mt-2 mb-3 py-0">
            <div className="row align-items-start justify-content-between my-0">
              <div className="col-8 col-sm-9 col-md-10">
                ...
                {trick.alias && trick.technicalName &&
                  <h3 className="text-muted fs-3 mb-0 mt-1">{trick.technicalName}</h3>
                }
              </div>
          ...
          {youtubeId &&
            <div className="ratio ratio-16x9">
              <YouTube className="video" videoId={youtubeId} opts={youtubeOpts} onReady={setupYoutubePlayer}
                       onEnd={restartVideo}/>
            </div>
          }
          {instagramLink &&
            <div align="center">
              <iframe className="insta-video" src={instagramLink} frameBorder="0" scrolling="no"
                      allowtransparency="true"
                      title="video"></iframe>
            </div>
          }

          {((trick.tips && trick.tips.length !== 0) || trick.recommendedPrerequisites.length !== 0) &&
            <hr className="my-3"/>
          }
            ...
            <div className={trick.tips ? "mt-3" : ""}>
              <div className="row">
                ...
                {trick.recommendedPrerequisites.map(recommendedTrick => {
                  if (recommendedTrick) {
                    return (
                      <div key={recommendedTrick.id} className="trick-container col-12" align="center">
                        <button className="btn preview-item skillFreq" freq={recommendedTrick.stickFrequency}
                                onClick={() => {
